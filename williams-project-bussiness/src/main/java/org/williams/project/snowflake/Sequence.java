...
public class Sequence {
     ...
     */
    private final long twepoch = 1257298974657L;
    /**
     * 机器标识位数
     */
        ...
        if (timestamp < lastTimestamp) {
            ...
            if (offset <= 5) {
                try {
                    ...
                    timestamp = timeGen();
                    if (timestamp < lastTimestamp) {
                        throw new SystemException(String.format("Clock moved backwards.  Refusing to generate id for %d milliseconds", offset));
                    }
                } catch (Exception e) {
